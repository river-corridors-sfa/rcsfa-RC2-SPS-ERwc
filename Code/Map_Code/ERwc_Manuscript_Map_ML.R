...
metadata_file = './Data/Multiple_linear_regression/v3_SFA_SpatialStudy_2021_SampleData/SPS_Sample_Field_Metadata.csv'
...
# Converting YRB_boundary to sf object explicitly
YRB_boundary_sf <- st_as_sf(YRB_boundary) #maggi
elevation_raw <- get_elev_raster(YRB_boundary_sf, z = 10) #maggi
...
ER_wc_map <- ggplot()+
  ...
  new_scale_fill()+
  geom_sf(data = sites, aes(color = Mean_ERwc, size = Mean_ERwc), show.legend = T) +
  #scale_fill_viridis(option = 'B', begin = 0.2)+
  scale_color_viridis(option = 'B', begin = 0.2#, name = expression(atop("Water Column Respiration", "(mg O"[2]*" L"^-1*" d"^-1*")"))
                      )+
  scale_size(range = c(1.5, 6), trans = 'reverse', name = NULL#,name = expression(atop("Water Column Respiration", "(mg O"[2]*" L"^-1*" d"^-1*")"))
             )+
  theme_map() +
  labs(x = "", y = "") +
  guides(
    color = guide_colorbar(
    title = expression(atop("Water Column Respiration", "(mg O"[2]*" L"^-1*" d"^-1*")")), 
    barwidth = 1, 
    barheight = 5, 
    title.position = "top",
    ...
    size = guide_legend(
      label = FALSE,
      title = "/n",# expression(atop("Water Column Respiration", "(mg O"[2]*" L"^-1*" d"^-1*")")), 
      direction = "vertical",# Align size points horizontally
      override.aes = list(color = scales::viridis_pal(option = "B", begin = 0.2, direction = -1)(3)),
      reverse = TRUE,# Use the color scale for points
      keyheight = unit(1.5, 'lines'))# Ensures they share the same legend
    ) +
  theme(
...
ggsave('./Data/Map_Layers/Archive_Intermediate_Files/Maggi_SPS_ER_Water_Column_Map.pdf',
       full,
       width = 8,
       height = 5
)
